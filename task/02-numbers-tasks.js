...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    const lenghtVectorA = Math.sqrt(x1 ** 2 + y1 ** 2);
    const lenghtVectorB = Math.sqrt(x2 ** 2 + y2 ** 2);
    const scalarVectors = x1 * x2 + y1 * y2;
    const cosVectors = scalarVectors / (lenghtVectorA * lenghtVectorB);
    return Math.acos(cosVectors);
}
...
function roundToPowerOfTen(num, pow) {
    const roundingDigit = Number(`1${'0'.repeat(pow)}`);
    return roundingDigit === 1 ? num : Math.round(num / roundingDigit) * roundingDigit;
}
